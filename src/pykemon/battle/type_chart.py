...
Only non-1.0 entries are stored; all missing keys default to 1.0.
Outer key = attacking move type.
Inner key = defending Pokemon type.
"""
...
def get_effectiveness(
    ...
    """Return the combined type effectiveness multiplier for a move against a defender."""
    chart = TYPE_CHART.get(move_type, {})
    e1 = chart.get(primary, 1.0)
    e2 = chart.get(secondary, 1.0) if secondary else 1.0
    return e1 * e2
